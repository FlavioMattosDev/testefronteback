    ...
    <>
      {products.map(product => (
        <div className="bg-white w-[144px] pb-3 rounded-md drop-shadow-md" key={product.id}>
          <img className="h-[96px] m-auto" src={product.imagem} alt={product.nome} />
          <h3 className="text-lg pl-3 font-bold text-blue-900">{product.nome}</h3>
          <p className="text-xs pl-3 text-slate-300 font-medium">{product.categoria}</p>
          <p className="text-orange-500 pl-3 font-semibold">R$ {product.preco}</p>
          <a
